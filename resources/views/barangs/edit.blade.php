...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center">
        ...
        <h3 class="text-lg font-semibold text-slate-700">Edit Data Barang</h3>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="merk_model" id="merk_model" value="{{ old('merk_model', $barang->merk_model) }}" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400" placeholder="Contoh: Asus VivoBook 14">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                            ...
                            <input type="number" name="jumlah_baik" id="jumlah_baik" min="0" value="{{ old('jumlah_baik', $barang->jumlah_baik) }}" class="w-full px-4 py-3 bg-white border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400 outline-none transition-all text-center text-lg font-bold text-emerald-700" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @php
                            $currentLokasiType = old('lokasi_type', $barang->lokasi_type === 'App\\Models\\Kelas' ? 'kelas' : ($barang->lokasi_type === 'App\\Models\\Lab' ? 'lab' : ''));
                        @endphp
                        <select name="lokasi_type" id="lokasi_type" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all mb-2" onchange="updateLokasiOptions()">
                            ...
                            <option value="kelas" {{ $currentLokasiType === 'kelas' ? 'selected' : '' }}>Kelas</option>
                            <option value="lab" {{ $currentLokasiType === 'lab' ? 'selected' : '' }}>Lab</option>
                        </select>
                        <select name="lokasi_id" id="lokasi_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" {{ $currentLokasiType ? '' : 'disabled' }}>
                ...
                {{-- Baris 7: Keterangan Mutasi --}}
                <div>
                    <label for="keterangan_mutasi" class="block text-sm font-semibold text-slate-700 mb-2">Keterangan Mutasi</label>
                    <textarea name="keterangan_mutasi" id="keterangan_mutasi" rows="3" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400" placeholder="Keterangan mutasi barang (opsional)">{{ old('keterangan_mutasi', $barang->keterangan_mutasi) }}</textarea>
...
<script>
    ...
    const currentLokasiId = {{ old('lokasi_id', $barang->lokasi_id ?? 'null') }};
    ...
    function updateLokasiOptions() {
        ...
        const data = type === 'kelas' ? kelasData : labsData;
        data.forEach(item => {
            const option = document.createElement('option');
            option.value = item.id;
            option.textContent = type === 'kelas' ? item.nama_kelas : item.nama_lab;
            if (item.id == currentLokasiId) option.selected = true;
