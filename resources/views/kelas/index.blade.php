        ...
        <div class="p-6 border-b border-slate-100">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <a href="{{ route('kelas.create') }}" class="inline-flex items-center px-3.5 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors text-sm font-medium shadow-sm">
            ...
            <div class="mt-4 flex flex-col md:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" x-model="search" placeholder="Cari nama kelas atau jurusan..."
                ...
                <select x-model="filterJurusan" class="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all min-w-[180px]">
                    ...
                    @php $jurusans = $kelas->pluck('jurusan.nama')->unique()->sort(); @endphp
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider" x-show="columns.includes('nama')">Kelas</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider" x-show="columns.includes('tingkat')">Tingkat</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors"
                        x-show="rowVisible('{{ addslashes($item->nama) }}', '{{ addslashes($item->jurusan->nama) }}')"
                        x-transition.opacity>
                        <td class="px-6 py-4 text-sm text-slate-600">{{ $loop->iteration }}</td>
                        <td class="px-6 py-4 text-sm font-semibold text-slate-800" x-show="columns.includes('nama')">{{ $item->nama }}</td>
                        <td class="px-6 py-4" x-show="columns.includes('tingkat')">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700">Tingkat {{ $item->tingkat }}</span>
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-slate-600" x-show="columns.includes('jurusan')">{{ $item->jurusan->nama }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end space-x-2">
                                <a href="{{ route('kelas.show', $item) }}" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-white rounded-lg border border-transparent hover:border-emerald-100 transition-all" title="Detail Barang">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="{{ route('kelas.edit', $item) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg border border-transparent hover:border-blue-100 transition-all" title="Edit">
                    ...
                    @if($kelas->isEmpty())
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-slate-400 italic">Belum ada data kelas.</td>
                    </tr>
                    @endif
                </tbody>
            </table>
...
function kelasTable() {
    return {
        ...
        columns: JSON.parse(localStorage.getItem('kelas_columns') || 'null') || ['nama', 'tingkat', 'jurusan', 'total_barang'],
        toggleColumn(key) {
            ...
            localStorage.setItem('kelas_columns', JSON.stringify(this.columns));
        },
