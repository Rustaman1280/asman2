...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center">
        ...
        <h3 class="text-lg font-semibold text-slate-700">Edit Data Kelas</h3>
    </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select name="jurusan_id" id="jurusan_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all" required>
                        @foreach($jurusans as $jurusan)
                            <option value="{{ $jurusan->id }}" {{ old('jurusan_id', $kelas->jurusan_id) == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama }} ({{ $jurusan->kode }})</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $kelas->nama) }}" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder:text-slate-400" required>
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all">Perbarui Kelas</button>
                </div>
            </div>
        </form>
    </div>
